...
class NBM(nn.Module):
    ...
    def __init__(self, nx, ny, nh, visible_unit_type="gaussian"):
        ...
        def forward(self, x, mc_steps=32, denoise=True):
            """
            Forward pass through the NBM, returning a sample from the visible units.

            Args:
                x (torch.tensor ~ (batch_size, nx)): Input tensor
                mc_steps (int): Number of Gibbs sampling steps
                denoise (bool): Whether to sample in a deterministic (mean-field) way

            Returns:
                torch.tensor ~ (batch_size, ny): A reconstructed sample from the visible units
            """
            bias = self.bias_net(x)
            precision = self.precision_net(x)
            weights = self.weights_net(x)

            # Start from the bias (mean) as initialization
            y_model = bias.clone()

            for _ in range(mc_steps):
                h_model = self._sample_hid(y_model, bias, precision, weights)
                y_model = self._sample_vis(h_model, bias, precision, weights, denoise=denoise)
    ...
    def compute_loss(self, y, x, mc_steps, fast=True):   
        """
        ...
        Args:
            ...
            x (torch.tensor ~ (batch_size, nx))
            mc_steps (int): number of Gibbs sampling steps
            fast (bool): if True, uses fewer MC steps and subsamples neg_phase

        Returns:
            dict: CD_loss, MSE_bias_loss, MSE_variance_loss
        """
        # Optimization 1: fewer sampling steps
        # nsteps = 4 if fast else max(1, mc_steps)
        # mc_steps = min(mc_steps, self.current_epoch + 1)  # ramps up with training
        ...
        if fast:
            # Optimization 2: CD loss altered, subsample both pos and neg phase
            idx = torch.randperm(y_flat.size(0))[: y_flat.size(0) // 2]
            y_flat = y_flat[idx]
            y_model = y_model[idx]
            bias = bias[idx]
