...
from utils import PlotSamples, accelerator, EpochTimerLogger
...
class TrainableNBM(LightningModule):
    ...
    def training_step(self, batch, batch_index):
        ...
        # losses = self.model.compute_loss(y, x, mc_steps=self.mc_steps, fast=True)

        losses = self.model.compute_loss(y, x,mc_steps=min(self.mc_steps, self.current_epoch + 1),fast=True)  

        # fast = self.current_epoch < 10  # Use full batch and steps after epoch 10
        # losses = self.model.compute_loss(y, x,mc_steps=min(self.mc_steps, self.current_epoch + 1),fast=fast)
...
def run(
    ...
    trainer = Trainer(
        ...
        logger=logger,
        callbacks=[lr_monitor, model_checkpoint, model_summary,timer_logger],
    )
...
if __name__ == "__main__":
    ...
    model_save_path = "trained_model_altered.pth"
    torch.save(model.state_dict(), model_save_path)
    print(f"Saved altered full model state dict to: {model_save_path}")


    # === Save NBM core model separately ===
    nbm_save_path = "nbm_model_altered.pth"
    torch.save(model.state_dict(), nbm_save_path)
    print(f"Saved altered NBM model state dict to: {nbm_save_path}")

    # === Save individual NBM submodules ===
    bias_path = "bias_net_altered.pth"
    torch.save(model.bias_net.state_dict(), bias_path)

    precision_path = "precision_net_altered.pth"
    torch.save(model.precision_net.state_dict(), precision_path)

    weights_path = "weights_net_altered.pth"
    torch.save(model.weights_net.state_dict(), weights_path)

    print("Saved altered  bias, precision, and weights nets separately.")
