...
if __name__ == "__main__":
    ...
    print(f"Saved full model state dict to: {model_save_path}")


    # === Save NBM core model separately ===
    nbm_save_path = "nbm_model.pth"
    torch.save(model.state_dict(), nbm_save_path)
    print(f"Saved NBM model state dict to: {nbm_save_path}")

    # === Save individual NBM submodules ===
    bias_path = "bias_net.pth"
    torch.save(model.bias_net.state_dict(), bias_path)

    precision_path = "precision_net.pth"
    torch.save(model.precision_net.state_dict(), precision_path)

    weights_path = "weights_net.pth"
    torch.save(model.weights_net.state_dict(), weights_path)

    print("Saved bias, precision, and weights nets separately.")
