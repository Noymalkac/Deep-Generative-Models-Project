...
# class EpochTimerLogger(Callback):
...
#         self.output_file = self.log_dir / "epoch_times.csv"
...
class EpochTimerLogger(Callback):
    ...
    def on_train_epoch_end(self, trainer, pl_module):
        ...
        # Create the output file only once, using trainer.logger.log_dir
        if self.output_file is None:
            log_dir = Path(trainer.logger.log_dir)
            self.output_file = log_dir / "epoch_times.csv"
            self.output_file.parent.mkdir(parents=True, exist_ok=True)
            with open(self.output_file, "w", newline="") as f:
                writer = csv.writer(f)
                writer.writerow(["epoch", "time_sec"])
